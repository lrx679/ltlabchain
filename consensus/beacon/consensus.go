...
const (
	initialDailyIssuance = uint64(5178)
	issuancePeriodDays   = uint64(30)
	secondsPerDay        = uint64(24 * 60 * 60)
	secondsPerBlock      = uint64(5)
	reductionNumerator   = int64(95)
	reductionDenominator = int64(100)
)
...
func calcBlockReward(chain consensus.ChainHeaderReader, header *types.Header) *uint256.Int {
	if header.Number == nil || header.Number.Sign() == 0 {
		return uint256.NewInt(0)
	}
	blocksPerDay := secondsPerDay / secondsPerBlock
	blocksPerPeriod := issuancePeriodDays * blocksPerDay
	if blocksPerPeriod == 0 {
		return uint256.NewInt(0)
	}
	period := header.Number.Uint64() / blocksPerPeriod

	// Start from 5178 ETH/day, convert it to a fixed per-block reward assuming 5s blocks,
	// then reduce the block reward by 5% every 30 days worth of blocks.
	dailyIssuanceWei := new(big.Int).Mul(new(big.Int).SetUint64(initialDailyIssuance), big.NewInt(params.Ether))
	blockRewardWei := new(big.Int).Div(dailyIssuanceWei, new(big.Int).SetUint64(blocksPerDay))
	if period > 0 {
		exponent := new(big.Int).SetUint64(period)
		numerator := new(big.Int).Exp(big.NewInt(reductionNumerator), exponent, nil)
		denominator := new(big.Int).Exp(big.NewInt(reductionDenominator), exponent, nil)
		blockRewardWei.Mul(blockRewardWei, numerator)
		blockRewardWei.Div(blockRewardWei, denominator)
	}
	if blockRewardWei.Sign() <= 0 {
		return uint256.NewInt(0)
	}
	return uint256.MustFromBig(blockRewardWei)
}
...
func (beacon *Beacon) Finalize(chain consensus.ChainHeaderReader, header *types.Header, state vm.StateDB, body *types.Body) {
	...
	blockReward := calcBlockReward(chain, header)
